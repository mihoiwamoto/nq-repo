import { PRODUCT_STATUS_LABELS } from "./mockData";
import iconArrowRight from "../../../assets/figma/icons/common/arrow-right.svg";

export function ProductProgressPanel({
  products,
  inspectedProducts,
  onClose
}: {
  products: any[];
  inspectedProducts: any[];
  onClose: () => void;
}) {
  const total = products.length;
  const inspectedCount = inspectedProducts.length;
          ...
          <div className="flex flex-col items-center w-full">
            {inspectedProducts.map((product, index) => (
              <div key={product.id} className="flex flex-col items-center w-full">
                ...
                {index < inspectedProducts.length - 1 && (
