...
export function SampleProgressPanel({
  samples,
  inspectedSamples,
  onClose
}: {
  samples: any[];
  inspectedSamples: any[];
  onClose: () => void;
}) {
  const total = samples.length;
  const inspectedCount = inspectedSamples.length;
      ...
      <div className="relative flex h-full">
        ...
        <div className="bg-[var(--semantic-background-page)] shadow-[0px_2px_3px_rgba(51,51,51,0.24)] rounded-lg h-full w-[348px] px-6 py-4 flex flex-col gap-4 overflow-y-auto overflow-x-hidden">
          ...
          <div className="flex flex-col items-center w-full">
            {inspectedSamples.map((sample, index) => (
              <div key={sample.id} className="flex flex-col items-center w-full">
                ...
                {index < inspectedSamples.length - 1 && (
