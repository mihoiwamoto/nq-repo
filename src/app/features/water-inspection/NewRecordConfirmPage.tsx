...
export function NewRecordConfirmPage() {
  ...
  const [recordedTime] = useState(() => new Date().toTimeString().slice(0, 5));
  ...
  const inspectorName = state.inspectorName ?? INSPECTOR_NAME;
  // 記録画面で項目ごとに付いた入力時刻をそのまま使う。
  // この画面で初めて出す項目（記録画面から時刻が来ていない場合）だけ今の時刻で補う。
  const fallback = `${state.date.replaceAll("-", "/")} ${recordedTime}`;
  /** 記録がある項目にだけタイムスタンプを出す（未入力の項目には出さない） */
  const stampIf = (recorded: unknown, time?: string) =>
    isUnrecorded(recorded) ? undefined : `${inspectorName} ${time || fallback}`;

  function handleSubmit() {
    if (!pointId || !state) return;
    addRecord(pointId, {
      inspector: state.inspectorName ?? INSPECTOR_NAME,
        ...
        <div className="bg-white flex flex-col items-start px-4 py-6 rounded-lg w-full max-w-[640px]">
          ...
          {state.checks.map((item) => (
            <CheckRow
              key={item.label}
              item={item}
              timestamp={stampIf(item.status, state.checkTimestamps?.[item.label])}
            />
          ))}
          <Row
            label="ph値"
            value={state.phValue}
            timestamp={stampIf(state.phValue, state.fieldTimestamps?.phValue)}
          />
          <div className="flex flex-col w-full">
            <Row label="残留塩素濃度(mg/ℓ)" value={state.residualChlorine} hideBorder />
            <ToggleRow label="塩素補充" checked={state.chlorineChecked} />
            {(!isUnrecorded(state.residualChlorine) || state.chlorineChecked) && (
              <p className="text-sm text-[var(--semantic-text-secondary)] text-right w-full font-normal pb-3">
                {stampIf(true, state.fieldTimestamps?.residualChlorine)}
              </p>
            )}
          ...
          <div className="flex flex-col w-full">
            ...
            {(!isUnrecorded(state.uvOperatingHours) || state.uvChecked) && (
              <p className="text-sm text-[var(--semantic-text-secondary)] text-right w-full font-normal pb-3">
                {stampIf(true, state.fieldTimestamps?.uvOperatingHours)}
              </p>
            )}
            <div className="border-t border-[#d0d0d0]" />
          </div>
