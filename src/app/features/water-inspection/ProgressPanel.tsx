import { POINT_INSPECTED_AT, type WaterPoint } from "./mockData";
...
export function ProgressPanel({
  ...
  const inspectedPoints = points.filter((point) => point.status !== "not_inspected");
  const total = points.length;
  const inspectedCount = inspectedPoints.length;
        ...
        <div className="bg-[var(--semantic-background-page)] shadow-[0px_2px_3px_rgba(51,51,51,0.24)] rounded-lg h-full w-[348px] px-6 py-4 flex flex-col gap-4 overflow-y-auto overflow-x-hidden">
          ...
          <div className="flex flex-col items-center w-full">
            {inspectedPoints.map((point, index) => (
              <div key={point.id} className="flex flex-col items-center w-full">
                ...
                {index < inspectedPoints.length - 1 && (
