...
export function NewRecordPage() {
  ...
  const [checks, setChecks] = useState<CheckItem[]>(
    CHECK_LABELS.map((label) => ({ label, status: null as any }))
  );
  const [phValue, setPhValue] = useState("");
  const [residualChlorine, setResidualChlorine] = useState("");
  const [chlorineChecked, setChlorineChecked] = useState(false);
  const [uvOperatingHours, setUvOperatingHours] = useState("");
  const [uvChecked, setUvChecked] = useState(false);
  const [uvIndicatorOk, setUvIndicatorOk] = useState(true);
  const [errorIndicatorOk, setErrorIndicatorOk] = useState(true);
