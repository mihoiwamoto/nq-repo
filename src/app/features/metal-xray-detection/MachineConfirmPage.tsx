...
import { MACHINES, MACHINE_RECORDS, RESULT_COLORS, RESULT_LABELS } from "./mockData";
...
export function MachineConfirmPage() {
  ...
  const state = location.state as { inspectionDate?: string; inspectorName?: string; hideAddButton?: boolean; fromProgress?: boolean } | null;
  ...
  return (
    <>
      ...
      <div className="shrink-0 bg-white shadow-[0px_-4px_16px_rgba(51,51,51,0.16)] px-6 py-6 flex items-center justify-center gap-6">
        <button
          type="button"
          onClick={() => navigate(detailPath, { state: { inspectorName: state?.inspectorName, fromProgress: state?.fromProgress } })}
        ...
        <button
          type="button"
          onClick={() => navigate(completePath, { state: { fromProgress: state?.fromProgress } })}
