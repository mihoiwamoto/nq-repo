...
function DateFilterPicker({
  ...
  const [currentDate, setCurrentDate] = useState(value ? parseDateKey(value) : new Date());
  ...
  const handleSelectDate = (day: number) => {
    onChange(toDateKey(currentDate.getFullYear(), currentDate.getMonth(), day));
    onClose();
  };
  ...
  return (
    <div ref={containerRef} className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-lg z-50 p-4 w-[280px]">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map((day, index) => {
          const dayDateString = day ? toDateKey(currentDate.getFullYear(), currentDate.getMonth(), day) : null;
