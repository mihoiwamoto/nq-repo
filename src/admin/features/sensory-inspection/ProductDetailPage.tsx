...
export function ProductDetailPage() {
  ...
  const { products, removeProduct } = useSensoryInspection();
      ...
      <div className="flex flex-col gap-4 items-start p-6">
        ...
        <div className="bg-white flex flex-col gap-3 items-start px-4 py-6 rounded-lg w-full">
          <div className="flex items-center justify-between w-full gap-4">
            <p className="text-xl text-[var(--semantic-brand-primary)] w-[152px]">検査製品名</p>
            <p className="text-xl text-[var(--semantic-text-primary)] flex-1 text-right">
              {product.name}
            </p>
          </div>
          {CRITERIA.map((criterion) => (
            <div key={criterion} className="w-full">
              <div className="border-t border-[#d0d0d0] w-full" />
              <div className="flex items-center justify-between w-full gap-4 pt-3">
                <p className="text-xl text-[var(--semantic-brand-primary)] w-[152px]">{criterion}</p>
                <p className="text-xl text-[var(--semantic-text-primary)]">
                  {product.criteria[criterion] ? "記録する" : "記録しない"}
                </p>
              </div>
            </div>
          ))}
