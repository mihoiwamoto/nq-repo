...
function averageScore(record: SensoryRecord, criterion: Criterion) {
  const scores = record.scoreEntries.map((entry) => entry.scores[criterion].score);
  return scores.reduce((sum, s) => sum + s, 0) / scores.length;
}

function isRecordAbnormal(record: SensoryRecord) {
...
export function DataListPage() {
  ...
  function handleDownload() {
    const header = ["日付", "検査製品名", ...CRITERIA, "検査結果", "確認者", "ステータス"];
