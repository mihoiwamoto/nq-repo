    ...
    <div>
      ...
      <Breadcrumb
        items={[
          ...
          { label: "工場選択", to: "/admin/confirmations/equipment-inspection" },
          { label: "データ一覧", to: `/admin/confirmations/equipment-inspection/factories/${factoryId}` },
          { label: "詳細" },
        ]}
      />
      <div className="flex flex-col gap-4 p-6">
        ...
        {record.sessions.length === 0 ? (
          ...
          record.sessions.map((session, sIndex) => (
            <div key={sIndex} className="bg-white flex flex-col gap-3 items-start px-4 py-6 rounded-lg w-full">
              <div className="flex items-center justify-between w-full">
                <p className="text-xl text-[var(--semantic-text-primary)]">実施区分</p>
                <p className="text-xl text-[var(--semantic-text-primary)]">{session.segment}</p>
              </div>
              {session.points.map((point, pIndex) => (
                <div key={pIndex} className="flex flex-col gap-3 items-start w-full">
                  <div className="bg-[var(--semantic-brand-primary)] flex items-center justify-between p-2 rounded-lg w-full">
                    <p className="text-xl text-white">点検箇所</p>
                    <p className="text-xl text-white">{point.location}</p>
                  </div>
                  <div className="flex flex-col gap-2 items-start px-2 w-full">
                    <p className="text-xl text-[var(--semantic-brand-primary)]">点検項目</p>
                        ...
                        <div className="flex items-center justify-between w-full">
                          ...
                          <span
                            className={`h-7 w-[88px] rounded-lg flex items-center justify-center text-base text-white ${
                              item.status === "ok" ? "bg-[#19c95f]" : "bg-[#f85c5c]"
                            }`}
                          >
                            {item.status === "ok" ? "正常" : "異常あり"}
                          </span>
                        </div>
                        {item.status === "ng" && (
