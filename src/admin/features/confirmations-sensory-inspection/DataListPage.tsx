...
import { CRITERIA, isAbnormalScore, type ConfirmStatus, type Criterion, type SensoryConfirmationRecord } from "./types";
...
function averageScore(record: SensoryConfirmationRecord, criterion: Criterion) {
  const scores = record.scoreEntries.map((entry) => entry.scores[criterion].score);
  return scores.reduce((sum, s) => sum + s, 0) / scores.length;
}

function isRecordAbnormal(record: SensoryConfirmationRecord) {
...
const COLUMNS = [
  ...
  { label: "検査製品名", width: "w-[280px]" },
  ...CRITERIA.map((c) => ({ label: c, width: "w-[64px]" })),
  { label: "検査結果", width: "w-[80px]" },
  { label: "確認者", width: "flex-1 min-w-[128px]" },
];
              ...
              <div className="flex flex-col gap-4 flex-1">
                <div className="flex gap-4 items-center">
                  ...
                  <Pulldown
                    ...
                    placeholder="検査製品名"
                    className="bg-white border border-[#d0d0d0] h-12 px-4 rounded-lg text-base text-[var(--semantic-text-primary)] w-[280px]"
                  />
                </div>
                ...
                filtered.map((record, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-[280px] flex items-center justify-start p-2 h-full text-sm text-[var(--semantic-text-primary)] text-left">
                        {record.productName}
                      </div>
                      {CRITERIA.map((c) => (
                        <div
                      ...
                      <div className="flex-1 min-w-[128px] flex items-center justify-center p-2 h-full text-sm text-[var(--semantic-text-primary)]">
                        {record.confirmer}
                      </div>
                    </div>
                  );
