      ...
      <div className="flex flex-col gap-6 p-6 min-h-screen">
        ...
        <div className="bg-white flex flex-col gap-3 items-start px-4 py-6 rounded-lg w-full">
          <div className="flex items-center justify-between w-full">
            ...
            <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">{record.implementer}</p>
          </div>
          <div className="border-t border-[#d0d0d0] w-full" />
          <div className="flex items-center justify-between w-full">
            <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">確認者</p>
            <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">{record.confirmer}</p>
          </div>
          <div className="border-t border-[#d0d0d0] w-full" />
          <div className="flex items-center justify-between w-full">
            <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">フロア名</p>
            <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">{record.floorName}</p>
          </div>
          <div className="border-t border-[#d0d0d0] w-full" />
          <div className="flex items-center justify-between w-full">
            <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">実施日</p>
        ...
        <div className="flex flex-col gap-3 items-start mt-4 mb-0">
                ...
                <div className="flex flex-col gap-3 items-start w-full">
                  {room.items.map((item, index) => (
                    <div key={item.name} className="flex flex-col gap-2 items-start w-full">
                      <div className="flex items-center justify-between w-full">
                        <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">
                          {item.name}
                        </p>
                        <span
                          className="h-7 w-[88px] flex items-center justify-center rounded-lg text-base font-semibold text-white"
                      ...
                      {item.status !== "normal" && (
                        <div className="flex flex-col gap-2 items-start px-2 text-base text-[#808080] w-full">
                          <p>内容：{item.content}</p>
                          <p>原因：{item.cause}</p>
                          <p>
                            対応：{item.actionType}
                      ...
                      <p className="text-sm text-[var(--semantic-text-secondary)] text-right w-full font-normal">
                        {record.implementer} {formatDate(record.date)} 10:15
                      </p>
        ...
        <div className="flex flex-col gap-2 items-start w-full">
          <p className="text-xl font-semibold text-[var(--semantic-text-primary)]">修理状況</p>
          <div className="bg-white flex items-center justify-between gap-4 px-4 py-6 rounded-lg w-full">
            ...
            <Link
              to={`/admin/ledger-management/glass-plastic/factories/${factoryId}/floors/${floorId}`}
